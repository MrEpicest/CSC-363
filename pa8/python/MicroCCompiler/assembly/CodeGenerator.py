  ...
  def postprocessBinaryOpNode(self, node: BinaryOpNode, left: CodeObject, right: CodeObject) -> CodeObject:
    ...
    if left.type is Scope.Type.INT and right.type is Scope.Type.INT:
      ...
      """if left.isVar():
        ...
        left.temp = self.generateTemp(Scope.Type.INT)
        co.code.append(Lw(left.temp, addr_temp, 0))
      ...
      if right.isVar():
        ...
        address = self.generateAddrFromVariable(right)
        addr_temp = self.generateTemp(Scope.Type.INT)
        co.code.append(La(addr_temp, address))

        right.temp = self.generateTemp(Scope.Type.INT)
        co.code.append(Lw(right.temp, addr_temp, 0))
