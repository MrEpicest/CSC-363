...
def parse_expression(ts: TokenStream) -> ASTNode:
    ...
    while ts.peek().tokentype != TokenType.EOF:
        ...
        if tok.tokentype == TokenType.LPAREN:
            ...
            tok = ts.read()
            next = ts.peek()
            if (next.tokentype == TokenType.INTLIT) or (next.tokentype == TokenType.VARREF) or (next.tokentype == TokenType.LPAREN) or (next.tokentype == TokenType.NEGATIVE) or(next.tokentype == TokenType.EOF):
                opstack.append(tok)
                continue
            raise ParseError("Expected lparen, intlit, negative, or varref after lparen")
        ...
        if tok.tokentype == TokenType.NEGATIVE:
            tok = ts.read()
            next = ts.peek()
            if next.tokentype == TokenType.LPAREN or next.tokentype == TokenType.VARREF or next.tokentype == TokenType.INTLIT:
                opstack.append(tok)
                continue
            raise ParseError("Expected lparen, intlit, or varref after negative")
        ...
        if tok.tokentype in operatortypes:
            ...
            #if len(valstack) == 0 and incoming.lexeme == '-':
                #negative = True

            #but what if we have something like a = 3 + -4?
    ...
    if len(valstack) != 1:
        raise ParseError(f"{opstack}{valstack[0].left}, {valstack[0].right} {valstack[1].left}, {valstack[1].right} Expression did not reduce to one AST")
...
def un_reduce(opstack: list, valstack: list) -> None:
    opstack.pop()
    value = valstack.pop()
    valstack.append(UnOpNode(value))
